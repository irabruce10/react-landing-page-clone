import Button from '../Button/Button';
import styles from './Header.module.scss';

function Header() {
  return (
    <header>
      <div className={styles.header_wrapper}>
        <nav>
          <ul>
            <li>
              <a>Women</a>
            </li>
            <li>
              <a>Men</a>
            </li>
            <li>
              <a>Kid</a>
            </li>
          </ul>
        </nav>

        <div>
          <a
            href="/"
            data-ga="nav-primary-gateway"
            class="style_home-link__ZBxjg">
            <div class="style_logotype__w9ARi">
              <svg
                width="298"
                height="48"
                viewBox="0 0 298 48"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                aria-labelledby="icon_R16qhn6__icon_title"
                data-testid="logo"
                role="img">
                <title id="icon_R16qhn6__icon_title">Stitch Fix</title>
                <path
                  d="M22.1118 23.7953L17.6531 22.4076C14.1953 21.3157 11.1469 20.0872 11.1469 17.6303C11.1469 15.8332 12.8303 14.3318 15.7649 14.3318C18.8133 14.3318 20.6559 15.9697 21.7934 18.7678L30.3014 15.3782C29.2095 11.0104 24.1137 6.11945 15.8104 6.11945C7.46161 6.11945 1.36493 11.0559 1.36493 18.3811C1.36493 26.0929 8.18957 28.7773 12.1706 29.9602L15.7877 31.0521C19.0408 32.0303 22.0436 33.1678 22.0436 35.8294C22.0436 38.2408 19.6322 39.6967 16.3791 39.6967C13.5355 39.6967 10.237 38.6048 8.6673 33.1678L0 36.0569C1.77441 43.3138 7.82559 48 16.5384 48C26.3659 48 31.9166 42.0853 31.9166 34.7602C31.9393 26.9801 25.4787 24.819 22.1118 23.7953Z"
                  fill="#001E60"></path>
                <path
                  d="M32.9175 15.3554H44.5876V47.0673H53.8691V15.3554H65.5165V6.89288H32.9175V15.3554Z"
                  fill="#001E60"></path>
                <path
                  d="M79.3251 6.89288H70.0436V47.0445H79.3251V6.89288Z"
                  fill="#001E60"></path>
                <path
                  d="M83.8522 15.3554H95.4996V47.0673H104.804V15.3554H116.451V6.89288H83.8522V15.3554Z"
                  fill="#001E60"></path>
                <path
                  d="M215.477 47.0673H224.758V31.2114H242.207V22.7488H224.758V15.3554H242.207V6.89288H215.477V47.0673Z"
                  fill="#001E60"></path>
                <path
                  d="M257.198 6.89288H247.917V47.0445H257.198V6.89288Z"
                  fill="#001E60"></path>
                <path
                  d="M297.85 47.1128L283.405 24.6825L281.699 22.0208L271.962 6.89288H260.997L275.784 30.1877L277.49 32.8493L286.544 47.1128H297.85Z"
                  fill="#001E60"></path>
                <path
                  d="M259.655 0H245.46V3.02559H259.655V0Z"
                  fill="#001E60"></path>
                <path
                  d="M81.7819 0H67.5867V3.02559H81.7819V0Z"
                  fill="#001E60"></path>
                <path
                  d="M186.517 22.7488H170.07V6.89288H160.789V47.0673H170.07V31.2114H186.517V47.0673H195.799V6.89288H186.517V22.7488Z"
                  fill="#001E60"></path>
                <path
                  d="M138.904 39.3327C133.718 39.3327 127.189 36.3526 127.189 27.2304C127.189 19.6095 131.761 14.673 138.313 14.673C142.817 14.673 145.774 17.0161 147.708 20.1555L156.557 16.8114C153.259 10.419 146.798 6.05121 138.472 6.05121C127.871 6.05121 117.475 13.0578 117.475 26.7526C117.475 39.583 126.552 47.909 138.904 47.909C146.525 47.909 153.805 44.7242 157.763 36.1706L149.027 33.2133C146.684 37.4901 142.976 39.3327 138.904 39.3327Z"
                  fill="#001E60"></path>
                <path
                  d="M296.258 6.89288H292.641L263.545 47.1128H267.162L296.258 6.89288Z"
                  fill="#001E60"></path>
              </svg>
            </div>
          </a>
        </div>

        <nav>
          <ul>
            <li>
              <a>Help</a>
            </li>
            <li>
              <a>Gift Card</a>
            </li>
            <li>
              <a>Sign In</a>
            </li>
          </ul>
        </nav>

        <Button />
      </div>
    </header>
  );
}

export default Header;
